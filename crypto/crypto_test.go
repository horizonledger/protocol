...
func TestCross(t *testing.T) {
	...
	pkh := "39663836643038313838346337643635396132666561613063353561643031356133626634663162326230623832326364313564366331356230663030613038"
	privk := PrivKeyFromHex(pkh)

	if pkh != PrivKeyToHex(keypair.PrivKey) {
		t.Error("priv key gen failed ", privk)
	}

	signature := SignMsgHash(keypair.PrivKey, message)
	sighex := SignatureToHex(signature)
	log.Println("sighex ", sighex)

	should_sighex := "3045022100b6789781f032512fc9ae06e9621ca4ce40d317a83a6b6f4ee1ad35942a3c928602204d8f03b330efc416b822447862333140d0acb3323d4575f1efba6e5418a036f7"
	if sighex != should_sighex {
		t.Error("fail")

	}
	verified := VerifyMessageSign(signature, keypair.PubKey, message)

	if !verified {
		t.Error("msg failed ", signature)
		// log.Println("keypair.PubKey ", keypair.PubKey)
		// log.Println("PubKeyToHex: ", PubKeyToHex(keypair.PubKey))
		// log.Println("PrivKey: ", PrivKeyToHex(keypair.PrivKey))
...
func TestBasicSign(t *testing.T) {
	...
	if !verified {
		t.Error("verify msg failed ", signature)
		// log.Println("keypair.PubKey ", keypair.PubKey)
		// log.Println("PubKeyToHex: ", PubKeyToHex(keypair.PubKey))
		// log.Println("PrivKey: ", PrivKeyToHex(keypair.PrivKey))
	...
	verifiedfalse := VerifyMessageSign(signature, keypair.PubKey, messagefalse)
	...
	verifiedother := VerifyMessageSign(signature, otherkeypair.PubKey, message)
	if verifiedother {
